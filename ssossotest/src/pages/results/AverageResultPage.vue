  ...
  <q-page class="row items-center justify-evenly absolute-top">
    <div class="row items-start">
      <q-card class="my-card" flat bordered>
        <q-img
          :src="`src/assets/images/average/${this.image}.jpeg`"
        />
        <q-card-section>
          <div class="text-overline text-orange-9">당신의 결과!</div>
          <div class="q-pa-md" style="max-width: 350px">
            <q-list bordered separator v-for="(item, i) in this.averageModel" :key="i">
              <q-item v-if="item.type = 'range'">
                <q-item-section>{{item.question}}</q-item-section>
                <q-item-section>{{item.result + item.answer[0].unit}}</q-item-section>
              </q-item>
